...
/* ── Slider ── */
function Slider({ value, min, max, step = 1, onChange, color = '#6ABDB2' }) {
    ...
    <div className="relative w-full" style={{ height: 28, display: 'flex', alignItems: 'center' }}>
      ...
      <div
        ...
        style={{
          ...
          background       : color,
          borderColor      : '#010D0D',
          boxShadow        : `0 0 0 3px ${color}30, 0 2px 8px rgba(0,0,0,0.5)`,
          pointerEvents    : 'none',
        }}
      />
    </div>
...
function StatCard({ icon: Icon, value, suffix, prefix, label, color, animate }) {
  return (
    <div
      ...
      style={{ background: '#011E1E', border: `1px solid ${color}20` }}
    >
      ...
      <div>
        ...
        <p className="text-xs font-medium" style={{ color: '#5A8A78' }}>{label}</p>
      </div>
    </div>
  )
}
...
function WhatIf({ hoursPerMonth }) {
  ...
  return (
    ...
    >
      <Zap size={18} className="flex-shrink-0 mt-0.5" style={{ color: '#6ABDB2' }} />
      <div>
        <p className="text-xs font-bold mb-1" style={{ color: '#6ABDB2' }}>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          <span
            ...
            style={{ background: 'rgba(106,189,178,0.12)', border: '1px solid rgba(106,189,178,0.25)', color: '#6ABDB2' }}
          >
            <TrendingUp size={12} />
          ...
          <p className="text-sm mx-auto" style={{ color: '#7A9E96', maxWidth: 500 }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-8">
          ...
          <div
            ...
            style={{ background: '#011E1E', border: '1px solid rgba(229,211,179,0.08)' }}
          >
            {/* Students slider */}
            <div>
              <div className="flex items-center justify-between mb-5">
                ...
                <span
                  ...
                  style={{ color: '#6ABDB2', background: 'rgba(106,189,178,0.10)', minWidth: 60, textAlign: 'center' }}
                >
                  {students}
                </span>
              </div>
              <Slider
                ...
                color="#6ABDB2"
              />
              <div className="flex justify-between mt-2.5 text-[10px]" style={{ color: '#5A8A78' }}>
                <span>15</span>
                <span>400</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: 'معلمون', value: roi.teachers, color: '#D9ACA3' },
                { label: 'حلقات',  value: roi.groups,   color: '#A3C4D9' },
              ].map(d => (
                <div
                  key={d.label}
                  className="rounded-xl px-4 py-3 text-center"
                  style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${d.color}18` }}
                >
                  <p className="font-black text-xl tabular-nums" style={{ color: d.color }}>
                    {visible ? <AnimCounter value={d.value} /> : d.value}
                  </p>
                  <p className="text-[11px] mt-0.5" style={{ color: '#5A8A78' }}>{d.label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-2">
              <p className="text-xs font-bold" style={{ color: '#7A9E96' }}>ما الذي يستغرق وقتك؟</p>
              {[
                { label: 'تسجيل الحضور يدوياً', mins: Math.round(students * 3), color: '#D9ACA3' },
                { label: 'توثيق جلسات التسميع', mins: Math.round(students * 3), color: '#A3C4D9' },
                { label: 'تواصل الأهالي (واتساب)', mins: Math.round(students * 2 + roi.teachers * 15), color: '#B5A3D9' },
                { label: 'تقارير وإدارة الكادر', mins: roi.teachers * 45 + 45, color: '#D9C8A3' },
              ].map(row => {
                const pct = Math.min(100, Math.round((row.mins / (roi.hoursPerWeek * 60)) * 100))
                return (
                  <div key={row.label}>
                    <div className="flex justify-between text-[11px] mb-1" style={{ color: '#5A8A78' }}>
                      <span>{row.label}</span>
          ...
          <div className="flex flex-col gap-5">
            ...
            >
              ...
              >
                ...
                <span className="text-center" style={{ color: '#6ABDB2' }}>مع إحكام</span>
              </div>
              {[
                ...
                >
                  ...
                  <span className="text-center font-bold" style={{ color: '#6ABDB2' }}>{row.after}</span>
                </div>
              ))}
            </div>
            ...
            <div
              ...
              style={{ background: 'rgba(2,115,104,0.08)', border: '1px solid rgba(2,115,104,0.20)' }}
            >
              <div>
              ...
              <Link
                ...
                style={{ background: 'linear-gradient(135deg, #027368 0%, #015a53 100%)', color: '#E5D3B3', boxShadow: '0 8px 24px rgba(2,115,104,0.32)' }}
              >
                ابدأ الآن
                <ArrowLeft size={14} />
              </Link>
